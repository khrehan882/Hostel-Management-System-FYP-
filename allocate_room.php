...
<html lang="en">
<head>
<title>Hostel Manaement System</title>
	...
	<header>
		<div class="container agile-banner_nav">
			<nav class="navbar navbar-expand-lg navbar-light bg-light">
				...
				<h1><a class="navbar-brand" href="home_manager.php">gomal<span class="display"></span></a></h1>
...
if(isset($_POST['submit'])) {
    // Retrieve hostel ID from session
    $hostel_id = $_SESSION['hostel_id'];

    // Query to select applications for the current hostel with status 1 (received)
    $query1 = "SELECT * FROM Application WHERE Hostel_id = '$hostel_id' AND Application_status = '1'";
    $result1 = mysqli_query($conn, $query1);
    ...
    while($row1 = mysqli_fetch_assoc($result1)) {
        // Retrieve student ID from the current application
        $student_id = $row1['Student_id'];

        // Query to find the minimum available room number for the hostel
        $query2 = "SELECT * FROM Room WHERE Room_No = (SELECT MIN(Room_No) FROM Room WHERE Allocated = '0' AND Hostel_id = '$hostel_id')";
        $result2 = mysqli_query($conn, $query2);

        // Check if the query executed successfully and if there are available rooms
        ...
        // Update application status and allocate room for the student
        ...
        // Check if the update query executed successfully
        if(!$result5) {
            echo "Error in query5: " . mysqli_error($conn);
            exit();
        }
    }
